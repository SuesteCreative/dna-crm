    ...
    const startScanner = async (mode: "environment" | "user") => {
        ...
        try {
            ...
            if (err?.name === "NotAllowedError" || err?.toString().includes("Permission denied")) {
                setError("Acesso à câmara negado. Por favor, permita o acesso nas definições do seu navegador.");
            } else if (err?.name === "NotFoundError" || err?.name === "DevicesNotFoundError") {
                setError("Nenhuma câmara detetada no seu dispositivo.");
            } else {
                setError("Erro ao iniciar a câmara. Tente recarregar a página ou verifique as permissões.");
            }
            setIsLoading(false);
        }
    };
