...
const getPricePerDay = (r: { period: string }, conc: { priceMorning: number; priceAfternoon: number; priceFull: number }) => {
...
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    const totalSpots = concession.spots.length || 1;
    ...
    const avgOccupancy = daysWithData.length
      ? daysWithData.reduce((s, set) => s + set.size, 0) / daysWithData.length : 0;
    ...
    const periodBreakdown = Object.entries(periodMap).map(([p, v]) => ({
      name: PERIOD_PT[p] ?? p, count: v.count, revenue: Math.round(v.revenue * 100) / 100,
    ...
    const bedBreakdown = Object.entries(bedMap).map(([b, v]) => ({
      name: BED_PT[b] ?? b, count: v.count, revenue: Math.round(v.revenue * 100) / 100,
    ...
    const salesByDayOfWeek = DOW_PT.map((d, i) => ({
      day: d, count: dowMap[i], revenue: Math.round(dowRevMap[i] * 100) / 100,
    }));
    ...
    for (const r of reservations) {
      ...
      if (freeDays > 0) {
        discountCount++;
        discountTotal += freeDays * getPricePerDay(r, concession);
      }
    }
